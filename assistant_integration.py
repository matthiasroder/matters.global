...
class AssistantIntegration:
    ...
    def run_assistant(self, thread_id, instructions=None):
        ...
        try:
            ...
            # Initialize a variable to store problem data from function calls
            self.latest_problem_data = None
            ...
            while True:
                ...
                if run.status == "completed":
                    ...
                    if run.required_action and run.required_action.type == "submit_tool_outputs":
                        ...
                        print(f"Run requires action with {len(tool_calls)} tool calls")
                        ...
                        for tool_call in tool_calls:
                            ...
                            if tool_call.type == "function":
                                ...
                                # If this is our target function, save the arguments
                                if func_name == "define_problem_parameters":
                                    self.latest_problem_data = json.loads(func_args)
                            
                            # Submit a response to continue
                            tool_outputs.append({
    ...
    def extract_tool_calls_from_run(self, thread_id, run_id):
        ...
        try:
            ...
            # Look for function call steps (compatible with v2 API structure)
            for step in run_steps.data:
                # v2 API might have a different structure
                if hasattr(step, 'type') and step.type == "tool_calls":
                    tool_calls.extend(step.step_details.tool_calls)
                elif hasattr(step.step_details, 'tool_calls'):
                    tool_calls.extend(step.step_details.tool_calls)
                # Check other possible locations in v2
    ...
    def extract_problem_from_run(self, thread_id, run_id):
        ...
        """
        # First check if we already captured problem data during run execution
        if hasattr(self, 'latest_problem_data') and self.latest_problem_data:
            print("Using problem data captured during run execution")
            return self.latest_problem_data
            
        # If not, try to extract it from the run's tool calls
        try:
            ...
            # As a last resort, check run steps
            steps = self.client.beta.threads.runs.steps.list(thread_id=thread_id, run_id=run_id)
            for step in steps.data:
                if step.type == "tool_calls":
                    for tool_call in step.step_details.tool_calls:
                        if (tool_call.type == "function" and 
                            tool_call.function.name == "define_problem_parameters"):
                            func_args = json.loads(tool_call.function.arguments)
                            return func_args
        except Exception as e:
            print(f"Error extracting problem from run: {e}")
            
        # If we reach here, no problem definition was found
        print("No problem definition found in the run")
        return None
...
if __name__ == "__main__":
    ...
    if not os.getenv("OPENAI_API_KEY"):
        ...
        problem = integration.create_problem_from_user_input(user_input)
